...
from src.calendar_app.models import CalendarEvent, Calendar, CalendarMonth
from src.calendar_app import utils, database as db
...
def sample_event_list(sample_calendar, sample_month):
    ...
    event1 = CalendarEvent(
        id="event1", calendar=sample_calendar, month=sample_month, title="Event 1",
        start_datetime=datetime(2025, 5, 10, 10, 0, 0, tzinfo=timezone.utc),
        end_datetime=datetime(2025, 5, 10, 11, 0, 0, tzinfo=timezone.utc), all_day=False
    )
    event2 = CalendarEvent(
        id="event2", calendar=sample_calendar, month=sample_month, title="Event 2",
        start_datetime=datetime(2025, 5, 11, 14, 0, 0, tzinfo=timezone.utc),
        end_datetime=datetime(2025, 5, 11, 15, 0, 0, tzinfo=timezone.utc), all_day=False
    )
    # Event with missing calendar color initially
    calendar_no_color = Calendar(calendar_id="cal_no_color", name="Needs Color", color_hex=None)
    event3 = CalendarEvent(
        id="event3", calendar=calendar_no_color, month=sample_month, title="Event 3 Needs Color",
        start_datetime=datetime(2025, 5, 12, 9, 0, 0, tzinfo=timezone.utc),
        end_datetime=datetime(2025, 5, 12, 9, 30, 0, tzinfo=timezone.utc), all_day=False
    )
...
def sample_google_data():
    ...
    return [
        ...
        },
        {
            ...
            'start_datetime': datetime(2025, 5, 16, 0, 0, 0, tzinfo=timezone.utc), # Often midnight for all-day
            'end_datetime': datetime(2025, 5, 17, 0, 0, 0, tzinfo=timezone.utc),   # Often midnight next day
            'all_day': True,
...
def test_add_events_inserts_new(mock_get_next_color, mock_db_module, mock_sqlite_connect, mock_db_connection, sample_event_list):
    ...
    for event in sample_event_list:
        ...
        if event.id == "event3":
             ...
             insert_calls.append(call(
                 ...
                 (event.calendar.calendar_id, event.calendar.name, mocked_color) # Expected assigned color
             ))
...
@patch('src.calendar_app.utils.db') # Mock the db module alias used in create_calendar_events...
def test_create_calendar_events_new_calendar(mock_db_module, sample_google_data, sample_month):
    ...
    created_calendar_mock = Calendar(calendar_id=new_cal_id, name=new_cal_name, color_hex=assigned_color)
    mock_db_module.get_calendar.side_effect = [None, created_calendar_mock] # First call returns None, second returns the object with color
    mock_db_module.add_calendar.return_value = None # Doesn't need to return anything specific

    events, calendars_changed = utils.create_calendar_events_from_google_data(google_data_new_cal, sample_month)
...
@patch('src.calendar_app.utils.db')
def test_create_calendar_events_existing_calendar_name_change(mock_db_module, sample_google_data, sample_month):
    ...
    existing_calendar_mock = Calendar(calendar_id=existing_cal_id, name=original_name, color_hex=existing_color)
    # Simulate the state after add_calendar updates the name but keeps the color
    updated_calendar_mock = Calendar(calendar_id=existing_cal_id, name=updated_name, color_hex=existing_color)

    mock_db_module.get_calendar.side_effect = [existing_calendar_mock, updated_calendar_mock]
    mock_db_module.add_calendar.return_value = None

    events, calendars_changed = utils.create_calendar_events_from_google_data(google_data_update_cal, sample_month)
    ...
    # Verify db interactions
    mock_db_module.get_calendar.assert_has_calls([call(existing_cal_id), call(existing_cal_id)])
    mock_db_module.add_calendar.assert_called_once()
    # Check the object passed to add_calendar
    added_calendar_arg = mock_db_module.add_calendar.call_args[0][0]
    assert isinstance(added_calendar_arg, Calendar)
    assert added_calendar_arg.calendar_id == existing_cal_id
    assert added_calendar_arg.name == updated_name # Updated name passed to add_calendar
    # add_calendar receives the object with the existing color when updating
    assert added_calendar_arg.color == existing_color


@patch('src.calendar_app.utils.db')
def test_create_calendar_events_existing_calendar_no_change(mock_db_module, sample_google_data, sample_month):
    ...
    existing_calendar_mock = Calendar(calendar_id=existing_cal_id, name=existing_name, color_hex=existing_color)
    mock_db_module.get_calendar.return_value = existing_calendar_mock # Always return the same object

    events, calendars_changed = utils.create_calendar_events_from_google_data(google_data_no_change, sample_month)

    assert calendars_changed is False # No calendar changes expected
    assert len(events) == 1
    event = events[0]
    assert event.id == google_data_no_change[0]['id']
    assert event.calendar == existing_calendar_mock # Should use the exact object returned by get_calendar
...
def test_add_events_updates_existing(mock_get_next_color, mock_db_module, mock_sqlite_connect, mock_db_connection, sample_event, sample_calendar, sample_month):
    """Test that add_events updates events using INSERT OR REPLACE."""
    mock_conn, mock_cursor = mock_db_connection
    mock_sqlite_connect.return_value = mock_conn
    mock_get_next_color.return_value = "#NEWCOLOR" # In case calendar color needs update
    ...
    for event in events_to_test:
        ...
        if not event.calendar.color:
            expected_calls.append(call(
                'INSERT OR REPLACE INTO Calendar (calendar_id, name, color) VALUES (?, ?, ?)',
                (event.calendar.calendar_id, event.calendar.name, mock_get_next_color.return_value)
            ))
