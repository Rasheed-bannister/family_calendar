...
class PIRSensor:
    ...
    def __init__(self, pin: int = 18, callback: Optional[Callable] = None, debounce_time: float = 2.0):
        """
        ...
        if not self.gpio_available:
            logging.warning("RPi.GPIO not available - PIR sensor will use simulation mode")
    ...
    def start_monitoring(self) -> bool:
        ...
        if self.gpio_available:
            ...
            # Simulation mode - start a thread that simulates motion detection
            self.monitor_thread = threading.Thread(target=self._simulate_motion, daemon=True)
            self.monitor_thread.start()
            logging.info("PIR sensor simulation mode started")
            return True
    ...
    def stop_monitoring(self):
        ...
        if self.gpio_available:
            ...
            logging.info("PIR sensor simulation mode stopped")
...
def initialize_pir_sensor(pin: int = 18, callback: Optional[Callable] = None) -> PIRSensor:
    """
