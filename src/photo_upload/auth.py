        ...
        try:
            ...
            if ip_address and token_data.get('ip'):
                if ip_address != token_data['ip']:
                    logger.warning(f"IP mismatch for token: expected {token_data['ip']}, got {ip_address}")
...
def init_token_manager(app):
    ...
    logger.info(f"Upload token manager initialized with {token_lifetime}s token lifetime")
...
def rate_limit_upload(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        identifier = (
            ...
            request.headers.get('X-Upload-Token') or
            request.remote_addr
        )
