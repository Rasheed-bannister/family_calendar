from flask import Blueprint, request, jsonify, render_template, current_app, url_for, make_response
import os
import uuid
import datetime
import socket
...
from .auth import require_upload_token, init_token_manager, token_manager, generate_upload_url, rate_limit_upload
...
def after_request(response):
    ...
    response.headers.add('Access-Control-Allow-Headers', 'Content-Type,Authorization,X-Upload-Token')
...
def optimize_image(image_path):
    ...
    try:
        ...
        if is_heic and not HEIF_AVAILABLE:
            logger.warning(f"Cannot process HEIC file {image_path} - pillow-heif not available")
        ...
        if image_path.lower().endswith(('.heic', '.heif')):
            try:
                os.remove(image_path)
                logger.info(f"Removed incompatible HEIC file: {os.path.basename(image_path)}")
            except:
                pass
            return None
        return image_path
...
def upload_photos():
    ...
    for file in files:
        ...
        if not allowed_file(file.filename):
            errors.append(f"Invalid file type: {file.filename}. Allowed: {', '.join(ALLOWED_EXTENSIONS)}")
            continue
        ...
        try:
            ...
            if optimized_path is None:
                errors.append(f"Unable to process {file.filename} - format not supported")
                continue
...
def test_token():
    ...
    return jsonify({"success": True, "message": "Token is valid!", "ip": request.remote_addr})
