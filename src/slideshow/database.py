...
def sync_photos(static_folder_path):
    ...
    try:
        ...
        current_files = {f for f in os.listdir(photos_dir)
        ...
        if files_to_add:
            ...
            cursor.executemany('INSERT OR IGNORE INTO background_photos (filename) VALUES (?)', [(f,) for f in files_to_add])
        ...
        if files_to_remove:
            ...
            cursor.executemany('DELETE FROM background_photos WHERE filename = ?', [(f,) for f in files_to_remove])
...
def get_random_photo_filename():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('SELECT filename FROM background_photos ORDER BY RANDOM() LIMIT 1')
