...
const Modal = (function () {
  ...
  return {
    ...
    show: function (eventData) {
      ...
      eventModal.querySelector(".modal-content").style.borderLeft = `5px solid ${
        eventData.color || "#ccc"
      }`;
    ...
    initAddChoreModal: function () {
      ...
      window.addEventListener("click", (event) => {
        ...
        const isVirtualKeyboard =
          event.target.closest("#virtual-keyboard") || event.target.closest(".virtual-keyboard");
      ...
      addChoreForm.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          if (response.ok) {
            closeModal();
            addChoreForm.reset();
            // Swap in the updated chores list rather than reloading:
            // a full reload here restarted the slideshow and closed
            // the UI the user was just interacting with.
            const swapped = await swapFragment("/chores/fragment", ".chores-list");
            if (!swapped) {
              // Fragment fetch failed; fall back to a reload so the
              // new chore is not silently missing from the display.
              location.reload();
            }
          } else {
