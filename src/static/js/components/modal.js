      ...
      addChoreForm.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          if (response.ok) {
            closeModal();
            addChoreForm.reset();
            // Update chores section dynamically instead of reloading
            if (window.updateChoresSection) {
              await window.updateChoresSection();
            } else {
              location.reload(); // Fallback if dynamic update not available
            }
          } else {
