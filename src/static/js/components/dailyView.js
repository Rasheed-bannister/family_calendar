...
const DailyView = (function () {
  ...
  function formatEventHTML(eventData) {
    let timeString =
      eventData.allDay === "true" ? "All Day" : `${eventData.startTime} - ${eventData.endTime}`;
    ...
    let locationString = eventData.location
      ? `<small>Location: ${eventData.location}</small><br>`
      : "";
    let descriptionString = eventData.description
      ? `<small>Notes: ${eventData.description}</small>`
      : "";
