...
class HealthMonitor:
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        if "system" in system_info:
            ...
            if len(recent_critical_errors) >= self.restart_threshold:
                health_status = "critical"
                issues.append(f"Too many critical errors ({len(recent_critical_errors)} in 10 minutes)")
        ...
        with self.lock:
            ...
            if is_critical:
                ...
                if self.should_restart():
                    logging.critical("Critical error threshold reached. Application restart required.")
    ...
    def _get_recent_critical_errors(self) -> list:
        ...
        return [error for error in self.critical_errors if error["timestamp"] > cutoff_time]
    ...
    def _cleanup_old_critical_errors(self):
        ...
        self.critical_errors = [error for error in self.critical_errors if error["timestamp"] > cutoff_time]
